...
(function () {
  ...
  BPC.get_summary = function(){

    BBClient.summary(function(summary){

      var demographics = {
        name: summary.demographics.name.given.join(" ") + " "+  summary.demographics.name.family,
        gender: summary.demographics.gender.toLowerCase(),
        birthday: summary.demographics.dob.toISOString()
      };
      ...
      summary.vitals.forEach(function(v){

        var systolic = byCode(v, "8480-6");
        var diastolic = byCode(v, "8462-4");
        var height = byCode(v, "8302-2");

        if (systolic.length > 0 && diastolic.length > 0) {
          vitals.bpData.push({
            vital_date: v.date.toISOString(),
    ...
    dfd.resolve(demographics, vitals);
